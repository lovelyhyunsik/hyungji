<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width= , initial-scale=1.0">
    <title>Document</title>
    <link rel="stylesheet" type="text/css"
        href="https://cdn.jsdelivr.net/gh/orioncactus/pretendard/dist/web/static/pretendard.css" />
    <link rel="stylesheet" href="css/reset.css">
    <link rel="stylesheet" href="css/common.css">
    <link rel="stylesheet" href="css/swiper-bundle.min.css" />
    <link rel="stylesheet" href="css/swiper.css">
    <link rel="stylesheet" href="css/order_list.css">
    <link rel="stylesheet" href="css/response.css">
    <script src="https://code.jquery.com/jquery-3.6.3.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/swiper@8/swiper-bundle.min.js"></script>
</head>
<body>
    <header id="header_type2"> 
        <div>
            <p>주문내역</p>
            <div class="header_ico_wrap">
            <a href="./bottom_gnb_mypage_member.php" class="ico_left_arrow"><span class="hide">뒤로가기</span></a>
                <div class="header_ico">
                    <a href="./search.php" class="ico_search"><span class="hide">검색아이콘</span></a>
                    <a href="./shoppingbag.php" class="ico_shoppingbag"><span class="hide">쇼핑백아이콘</span></a>
                </div>
            </div>
        </div>
    </header> 
    <div class="orderListTopWrap">
        <ul class="orderListTop">
            <li>
                <a href="">
                    <p class="text">배송중</p>
                    <p class="number">0</p>
                </a>
            </li>
            <li>
                <a href="">
                    <p class="text">배송완료</p>
                    <p class="number">0</p>
                </a>
            </li>
            <li>
                <a href="">
                    <p class="text">취소 / 반품 / 교환</p>
                    <p class="number">0</p>
                </a>
            </li>
        </ul>
        <div class="dateBtn">
            <div class="dateBtnContainer">
                <button class="fifteenDay on">15일</button>
                <button class="oneMonth">1 개월</button>
                <button class="threeMonth">3 개월</button>
            </div>
        </div>
    </div>
    <section id="orderItemWrap">
        <ul>
            <a href="./order_date_detail_many.php" class="dateDetail">
                <span class="itemDate">2022.12.01</span>
                <img src="./img/ic-arrow-right.png" alt="">
            </a>
            <li>
                <a href="">
                    <img src="./img/Frame 2642.png" alt="">
                    <div class="itemText">
                       <div class="state">
                           결제완료<span>12.01 (목) 도착</span>
                       </div>
                       <div class="brandItemName">
                           <span class="brandName">CHATELAINE</span><span class="itemName">브러쉬 립조직 하이넥 티셔츠</span>
                       </div>
                       <div class="itemOpt">
                           <span class="color">네이비</span>/<span class="size">55</span>/<span class="count">1개</span>
                       </div>
                       <div class="itemPrice">147,000원</div>
                    </div>
                </a>
            </li>
            <li>
                <a href="">
                    <img src="./img/Frame 2642.png" alt="">
                    <div class="itemText">
                       <div class="state">
                           결제완료<span>12.01 (목) 도착</span>
                       </div>
                       <div class="brandItemName">
                           <span class="brandName">CHATELAINE</span><span class="itemName">브러쉬 립조직 하이넥 티셔츠</span>
                       </div>
                       <div class="itemOpt">
                           <span class="color">네이비</span>/<span class="size">55</span>/<span class="count">1개</span>
                       </div>
                       <div class="itemPrice">147,000원</div>
                    </div>
                </a>
            </li>
            <li class="shipping"><!-- 배송중:shipping , 구매확정:confirm-->
                <a href="">
                    <img src="./img/Frame 2642.png" alt="">
                    <div class="itemText">
                       <div class="state">
                           배송중<span>12.01 (목) 도착</span>
                       </div>
                       <div class="brandItemName">
                           <span class="brandName">CHATELAINE</span><span class="itemName">브러쉬 립조직 하이넥 티셔츠</span>
                       </div>
                       <div class="itemOpt">
                           <span class="color">네이비</span>/<span class="size">55</span>/<span class="count">1개</span>
                       </div>
                       <div class="itemPrice">147,000원</div>
                    </div>
                </a>
            </li>
        </ul>
        <ul>
            <a href="" class="dateDetail">
                <span class="itemDate">2022.11.25</span>
                <img src="./img/ic-arrow-right.png" alt="">
            </a>
            <li>
                <a href="">
                    <img src="./img/Frame 2642.png" alt="">
                    <div class="itemText">
                       <div class="state">
                           결제완료<span>12.01 (목) 도착</span>
                       </div>
                       <div class="brandItemName">
                           <span class="brandName">CHATELAINE</span><span class="itemName">브러쉬 립조직 하이넥 티셔츠</span>
                       </div>
                       <div class="itemOpt">
                           <span class="color">네이비</span>/<span class="size">55</span>/<span class="count">1개</span>
                       </div>
                       <div class="itemPrice">147,000원</div>
                    </div>
                </a>
            </li>
            <li class="confirm">
                <a href="">
                    <img src="./img/Frame 2642.png" alt="">
                    <div class="itemText">
                       <div class="state">
                           구매확정<span>12.01 (목) 도착</span>
                       </div>
                       <div class="brandItemName">
                           <span class="brandName">CHATELAINE</span><span class="itemName">브러쉬 립조직 하이넥 티셔츠</span>
                       </div>
                       <div class="itemOpt">
                           <span class="color">네이비</span>/<span class="size">55</span>/<span class="count">1개</span>
                       </div>
                       <div class="itemPrice">147,000원</div>
                    </div>
                </a>
            </li>
            <li>
                <a href="">
                    <img src="./img/Frame 2642.png" alt="">
                    <div class="itemText">
                       <div class="state shipping">
                           배송중<span>12.01 (목) 도착</span>
                       </div>
                       <div class="brandItemName">
                           <span class="brandName">CHATELAINE</span><span class="itemName">브러쉬 립조직 하이넥 티셔츠</span>
                       </div>
                       <div class="itemOpt">
                           <span class="color">네이비</span>/<span class="size">55</span>/<span class="count">1개</span>
                       </div>
                       <div class="itemPrice">147,000원</div>
                    </div>
                </a>
            </li>
        </ul>
    </section>
</body>
<script>
    $(".dateBtnContainer>button").click(function(){
        $('.dateBtnContainer>button').removeClass('on')
        $(this).addClass('on')
    })
</script>
</html>